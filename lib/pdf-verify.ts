          ...
          for (const field of skillsAwareFields) {
            try {
              ...
              if (value) {
                ...
                decodedValue = decodedValue.replace(/#([0-9A-Fa-f]{2})/g, (match, hex) => {
...
export async function verifyPdfSignature(
  ...
  try {
    ...
    if (signature === expectedSignature) {
      ...
      if (textData.extractedData.skillCode && textData.extractedData.skillCode !== skillCode) {
        differences.push({
          field: 'Skill Code',
      ...
      if (textData.extractedData.claimantName && textData.extractedData.claimantName !== claimantName) {
        differences.push({
          field: 'Claimant Name',
      ...
      if (textData.extractedData.endorserName && textData.extractedData.endorserName !== endorserName) {
        differences.push({
          field: 'Endorser Name',
...
export async function verifyPdfBasic(pdfBuffer: Buffer): Promise<{
  ...
  try {
    ...
    if (!metadata.creator?.includes('SkillsAware') && !hasSkillsAwareSignature) {
      return {
        valid: false,
        message: 'This is not a SkillsAware PDF - no SkillsAware signature or creator information found',
        metadata,
      };
    }
        ...
        changes: [
          {
            ...
            description: 'The PDF was edited with another application, which modified the creator metadata'
          }
        ],
        extractedData: textData.extractedData,
        warning: 'This PDF has been modified after issuance. The creator field has been changed, indicating the document was opened and saved in another application.',
      };
    ...
    if (storedCredentialData && storedContentHash) {
      try {
        ...
        const expectedHash = crypto.createHash('sha256').update(contentPayload).digest('hex');
        ...
        if (expectedHash !== storedContentHash) {
          ...
          return {
            valid: false,
            message: '⚠️ METADATA TAMPERING DETECTED - The credential data stored in the PDF metadata has been modified.',
            metadata,
            tamperDetails: {
              detected: true,
              metadataTampered: true,
              warning: 'The cryptographic hash of the stored credential data does not match. This indicates the metadata has been altered.',
            },
          };
        }
        ...
        if (originalData.skillName) {
          const skillNamePattern = new RegExp(`Skill[:\\s]+${originalData.skillName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'i');
        ...
        if (originalData.skillCode) {
          const skillCodePattern = new RegExp(`Skill\\s+Code[:\\s]+${originalData.skillCode.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'i');
        ...
        if (originalData.skillDescription) {
          ...
          const escapedDesc = originalData.skillDescription.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
          // Allow for some whitespace variation but check substantial presence
          const descWords = originalData.skillDescription.split(/\s+/).filter(w => w.length > 3).slice(0, 5);
          const foundWords = descWords.filter(word => {
            const wordPattern = new RegExp(word.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
        ...
        if (originalData.claimantName) {
          const claimantPattern = new RegExp(`Claimant[:\\s]+${originalData.claimantName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'i');
        ...
        if (originalData.narrative) {
          const narrativeWords = originalData.narrative.split(/\s+/).filter(w => w.length > 3).slice(0, 5);
        ...
        if (originalData.endorserName) {
          const endorserPattern = new RegExp(`Endorsement\\s+by[:\\s]+${originalData.endorserName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'i');
        ...
        if (originalData.bonaFides) {
          const bonaFidesWords = originalData.bonaFides.split(/\s+/).filter(w => w.length > 3).slice(0, 5);
        ...
        if (originalData.endorsementText) {
          const endorsementWords = originalData.endorsementText.split(/\s+/).filter(w => w.length > 3).slice(0, 5);
        ...
        const signatureSection = textData.fullText.match(/Digital\s+Signature[:\s]+(.*?)(?=This\s+is\s+a\s+digitally|Generated\s+with|$)/is);
        ...
        if (originalData.evidence && Array.isArray(originalData.evidence) && originalData.evidence.length > 0) {
        ...
        if (differences.length > 0) {
          const tamperDetails = {
            ...
            changes: differences,
            originalData,
            warning: '⚠️ CONTENT TAMPERING DETECTED - The PDF text has been modified. Expected values from the original credential are missing from the PDF.',
          };
    ...
    if (storedJwt && !storedCredentialData) {
      try {
        ...
        try {
          ...
          if (jwtPayload.skill_code && textData.extractedData.skillCode &&
          ...
          if (jwtPayload.claimant_name && textData.extractedData.claimantName &&
          ...
          if (jwtPayload.endorser_name && textData.extractedData.endorserName &&
          ...
          if (differences.length > 0) {
            const tamperDetails = {
              ...
              warning: '⚠️ CONTENT TAMPERING DETECTED - The PDF content has been modified. The text in the PDF doesn\'t match the original data stored in the cryptographic signature.',
            };
          ...
          return {
            valid: false,
            message: '⚠️ JWT VERIFICATION FAILED - The cryptographic token in this PDF is invalid or has been tampered with.',
            metadata,
            tamperDetails: {
              detected: true,
              jwtError: String(jwtError),
              warning: 'The JWT token stored in the PDF metadata could not be verified. This indicates the PDF may have been tampered with.',
            },
          };
        }
      } catch (error) {
