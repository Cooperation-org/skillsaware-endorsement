...
async function test() {
  ...
  const original = fs.readFileSync(
    'E:/skillsaware-endorsement/public/ICTDSN403-91ff17dc-2b54-45dc-9079-3d439acdad47.pdf'
  )
  const originalText = (await pdf(original)).text

  const signature = 'Muhammad Hany'
  const signatureSection = originalText.match(
  ...
  const edited = fs.readFileSync(
    'E:/skillsaware-endorsement/public/ICTDSN403-91ff17dc-2b54-45dc-9079-3d439acdad47 (1).pdf'
  )
  const editedText = (await pdf(edited)).text

  const editedSignatureSection = editedText.match(
    /Digital\s+Signature[:\s]+(.*?)(?=This\s+is\s+a\s+digitally|Generated\s+with|$)/is
  )
  ...
  if (editedSignatureSection) {
    console.log(
      ...
      JSON.stringify(editedSignatureSection[1].trim())
    )
    console.log(
      'Contains original signature?',
      editedSignatureSection[1].includes(signature)
    )
  }
  ...
  console.log(
    'Original PASSES:',
    signatureSection && signatureSection[1].includes(signature)
  )
  console.log(
    'Edited FAILS:',
    !editedSignatureSection || !editedSignatureSection[1].includes(signature)
  )
}
