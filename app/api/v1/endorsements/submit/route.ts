...
export async function POST(request: NextRequest) {
  try {
    ...
    // Upload to S3
    const jsonKey = `${tenant.s3_prefix}/${payload.claim_id}/claim.obv3.json`;
    const pdfKey = `${tenant.s3_prefix}/${payload.claim_id}/claim.pdf`;
    ...
    await Promise.all([
      uploadToS3(jsonUrl, JSON.stringify(achievementCred, null, 2), 'application/json'),
